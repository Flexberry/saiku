...
r.value===q[v].value,R=!q,G=!1;if(Settings.ALLOW_TABLE_DATA_COLLAPSE)if(0<w&&w<Q-1){if(null==x[ROWS]||v<=S-x[ROWS].length-1){var A=!0;x[COLUMNS]&&0<=N&&v<=isNextTotalsRow(N+1,y,I,x,B)&&(G=!0,A=!1);if(A&&u[v].value==r.value)if(0<v)for(A=0;A<v;A++)if(u[A].value==e[w][A].value)G=!0;else{G=!1;break}else G=!0}}else 0<w&&w==Q-1&&x[COLUMNS]&&0<=N&&v<=isNextTotalsRow(N+1,y,I,x,B)&&(G=!0);u=r.value;"All "==u.substr(0,4)&&"s"==u.substr(-1)&&"All"in Saiku.i18n.po_file&&(u=Saiku.i18n.po_file.All+u.substr(3,u.length-
...
q+'" '+(0<G?' colspan\x3d"'+G+'"':"")+"\x3e"+u+"\x3c/th\x3e"}else if("ROW_HEADER_HEADER"===r.type){u=[];if(_.contains(h,r.value)){for(q=0;q<h.length;q++)h[q]===r.value&&u.push(q);u.push(v)}q=r.value;q in Saiku.i18n.po_file&&(q=Saiku.i18n.po_file[q]);d+='\x3cth class\x3d"row_header"\x3e'+(B?"\x3cdiv\x3e"+q+"\x3c/div\x3e":q)+"\x3c/th\x3e";h.push(r.value);k.push({name:r.value,hierName:function(a){a=a.properties.hierarchy;return a.replace(/[\[\]]/gi,"").split(".")[1]?a.replace(/[\[\]]/gi,"").split(".")[1]:
